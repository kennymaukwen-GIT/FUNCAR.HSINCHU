...
function carCard(car){
  ...
  return `
      <a href="${href}" class="car-card" data-brand="${esc(car.brand)}">
        <div class="car-card-media"><img class="car-card-img" src="${photoUrl(car,0)}" alt="${esc(car.title)} ${esc(car.subtitle)}"></div>
...
const brandCards = () => SHOWCASE_BRANDS.map(b => {
  ...
  return `
      <div class="showcase-slide" data-title="${esc(b.label)}">
        <a class="showcase-card brand-card" href="${href}" aria-label="${esc(b.label)}"><span class="brand-logo-text">${esc(b.label)}</span></a>
      </div>`;
}).join('') + '\n  ';
